...
**三类报告已收口为 `卡顿报告` / `轻微干扰` / `系统压力`**。`卡顿报告` 保留明显卡顿与强解释事件；`轻微干扰` 专门收短暂抖动、瞬时干扰、弱证据事件；`系统压力` 收资源压力优先、未必形成明显卡顿的事件。

**`轻微干扰` 使用慢指数退却**。它和 `系统压力` 一样走 `15s / 30s / 60s / ...` 这套退却，不再和 `卡顿报告` 共用快退却；筛选栏、历史读取、清空全部、旧数据回填都已补齐。

**`DISPLAY_PIPELINE` 在 `卡顿报告` 下单独使用专用退却**。普通 `卡顿报告` 仍走 `2s / 4s / 8s / 16s / 32s / 64s`，但 `detection_source in {"frame", "compat"}` 且 `category == "DISPLAY_PIPELINE"` 时，会改走专门的 `10s / 20s / 40s / 60s / 90s`。代码入口在 `ui/main_window.py` 的 `_report_gate_for_event()`；如果后续要继续调报告频率，先看这里，不要只改通用 `STUTTER_ALERT_INTERVALS_S`。

**切换事件页时减少空白与顿挫**。历史列表现在是后台先取数，再在 UI 侧一次性替换，不再先清空再等数据回来；`replace_events()` 用批量重建降低切页空窗。

**设置页新增了显式“保存设置”按钮**。数值改动仍保留 500ms 去抖自动保存，但现在用户也可以手动点保存；“恢复默认阈值”依旧立即生效并立即落盘。

**设置页切换时的轻微卡顿主要来自页面本身较重**。当前不是检测逻辑在切页时额外执行，而是设置页控件较多、首次显示会发生布局和重绘开销，后续如有需要可再做懒加载或拆页优化。

**`DISPLAY_PIPELINE` 改为“更高门槛 + 连续性优先 + 大异常直通”**。不再因为单帧轻微上屏波动就立刻报卡顿：现在先要求 `displayed_time` 超过更保守的基线阈值，再满足更高的独立超额条件（默认至少比帧成本多出 12ms 或达到 1.8x），并优先要求最近 10 个上屏帧里出现连续 2 次以上异常；只有超大异常（约 42ms+ 超额或 3x 级别）才允许单次直通。

**`DISPLAY_PIPELINE` 现在会按强弱分流到两个标签页**。同样保留 `DISPLAY_PIPELINE` 分类名，但短暂、无掉帧、无强异常、超额较小的上屏问题会落到 `轻微干扰`；带掉帧、强异常、较大超额或重复严重异常的则继续落到 `卡顿报告`。这样既保留原因解释的一致性，也让用户能一眼区分“偶发上屏抖动”和“真正值得关注的显示卡顿”。

**帧检测现在分成两条并行线**。一条是原有的“单帧严重卡顿”线，继续负责 `FRAME_STUTTER` / `FRAME_FREEZE` / 严重 `DISPLAY_PIPELINE`；另一条是新增的“短时连续变卡”线，用 0.55s / 1.0s 滚动窗口观察平均帧时、P95、慢帧占比和有效 FPS 跌幅，在没有超长单帧的情况下识别“这一小段时间突然只剩几帧”的短时连续掉速，并可落为 `FRAME_PACING_COLLAPSE` 或被归入 `轻微干扰`。

**设置页新增两个高层灵敏度**。`明显卡顿灵敏度` 控制单帧严重卡顿与严重上屏异常的阈值缩放；`连续波动灵敏度` 控制短时连续掉速和轻微波动聚类的阈值缩放。两者默认 `1.00x`（平衡），推荐参考：敏感 `0.85x`、平衡 `1.00x`、保守 `1.20x`。数值越小越容易触发，数值越大越保守。
